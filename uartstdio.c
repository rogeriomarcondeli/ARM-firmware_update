...
UARTPrimeTransmit(unsigned long ulBase)
{
    ...
    if(!TX_BUFFER_EMPTY)
    {
        ...
        IntDisable(g_ulUARTInt[g_ulPortNum]);
        ...
        IntEnable(g_ulUARTInt[g_ulPortNum]);
    }
}
...
UARTStdioInitExpClk(unsigned long ulPortNum, unsigned long ulBaud)
{
    ...
    UARTConfigSetExpClk(g_ulBase, SysCtlClockGet(SYSTEM_CLOCK_SPEED), ulBaud,
    ...
    UARTIntDisable(g_ulBase, 0xFFFFFFFF);
    UARTIntEnable(g_ulBase, UART_INT_RX | UART_INT_RT);
    IntEnable(g_ulUARTInt[ulPortNum]);
#endif
...
UARTStdioIntHandler(void)
{
    ...
    ulInts = UARTIntStatus(g_ulBase, true);
    UARTIntClear(g_ulBase, ulInts);
    ...
    if(ulInts & (UART_INT_RX | UART_INT_RT))
    {
        ...
        while(UARTCharsAvail(g_ulBase))
        {
            // Read a character
            lChar = UARTCharGetNonBlocking(g_ulBase);
